						...
						<div class="row">
							<div class="col-xs-12">
								...
								<div class="alert alert-block alert-success">
									...
									。注意：删除属性时所有附带的学生记录也都会被清空.	
								</div>

								<div class="hr hr32 hr-dotted"></div>

								<div class="row">
                  <form action="__APP__/Ctr/saveCtr" method="post">
                    <table class="table table-striped table-bordered table-hover" id="ctr_table">
                      <thead>
                        <th>属性名称</th>
                        <th>类型</th>
                        <th>操作</th>
                      ...
                      <tbody>
                        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td>
                              <input name='title[]' type="text" value="<?php echo ($vo["title"]); ?>" required="required">
                              <input type="hidden" name="id[]" value="<?php echo ($vo["ctr_id"]); ?>"/>
                            </td>
                            <td>
                              <select  name='type[]' >
                                <?php switch($vo["type"]): case "0": ?><option value="0" selected='selected'>文本</option>
                                    <option value="1" >数值</option><?php break;?>
                                  <?php case "1": ?><option value="0" >文本</option>
                                    <option value="1" selected='selected'>数值</option><?php break; endswitch;?>
                              </select>
                            </td>
                            <td>
                              <div class="btn btn-xs btn-danger" onclick="location='__APP__/Ctr/del_ctr/id/<?php echo ($vo["ctr_id"]); ?>'">
                    ...
                    <input type="hidden" name="school_id" value="<?php echo ($vo["school_id"]); ?>"/>
                    <div class="btn" onclick="addRow()">新增</div>
