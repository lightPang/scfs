	...
	<body>
		...
		<div class="main-container" id="main-container">
			...
			<div class="main-container-inner">
                ...
                <form action="__APP__/info/save_data" method="post" >
                  <div class="row">
                    <table class="table table-striped table-bordered table-hover" id="info_table">
                      <thead>
                        <?php if(is_array($head_data)): $i = 0; $__LIST__ = $head_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$th): $mod = ($i % 2 );++$i;?><th><?php echo ($th["title"]); ?></th><?php endforeach; endif; else: echo "" ;endif; ?>
                        <th>操作</th>
                      </thead>
                      <tbody>
                         <?php if(is_array($body_data)): $i = 0; $__LIST__ = $body_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bd): $mod = ($i % 2 );++$i;?><tr>
                          <?php if(is_array($bd)): $i = 0; $__LIST__ = $bd;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$th): $mod = ($i % 2 );++$i;?><td>
                              <input type="text" value="<?php echo ($th["content"]); ?>" name="content[]"/>
                              <input type="hidden" value="<?php echo ($th["info_id"]); ?>" name="info_id[]"/>
                             </td><?php endforeach; endif; else: echo "" ;endif; ?>
                          <td>
                              <div class="btn btn-xs btn-danger" onclick="location='__APP__/Info/del_record/stu_id/<?php echo ($th["stu_id"]); ?>'">
                                <i class="icon-trash bigger-120"></i>
                                <input type="hidden" name="stu_id[]" value="<?php echo ($th["stu_id"]); ?>" required="required"/>
                              </div>
                            </td>
                  ...
                  <?php if(is_array($head_data)): $i = 0; $__LIST__ = $head_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$th): $mod = ($i % 2 );++$i;?><input type="hidden" name="ctr_id[]" value="<?php echo ($th["ctr_id"]); ?>">
					<input type="hidden" name="type[]" value="<?php echo ($th["type"]); ?>"><?php endforeach; endif; else: echo "" ;endif; ?>
                  <input type="hidden" name="school_id" value="<?php echo ($th["school_id"]); ?>">
                  <div class="btn" onclick="addInfoRow()">新增</div>
